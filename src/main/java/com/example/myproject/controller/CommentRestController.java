...
// 만약, 모든 Controller가 RestController로 구성되면 /api는 생략하고 설계한다.
@RestController
...
public class CommentRestController {
    ...
    public ResponseEntity<Integer> create(@RequestBody CommentDTO commentDTO) {
        // @RequestBody : JSON 형태로 클라이언트에서 전송된 데이터를 Java 객체로 바인딩 해주는 역할
        int result = commentService.save(commentDTO);
        // ReseponseEntity<T> : 상태 코드, 헤더, 데이터를 직접 제어하는 객체
