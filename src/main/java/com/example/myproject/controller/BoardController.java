...
// Presentation Layer(표현 계층) : 클라이언트의 요청/응답을 처리
@Controller
@RequiredArgsConstructor
@RequestMapping("/board") // localhost:9090/board
public class BoardController {
    ...
    // application.properties에서 정의된 파일 업로드 경로를 uploadDir 변수에 주입
    ...
    public void listBoards(
        ...
        // Pageable에 page 변수가 적용되어 있기 때문에 parameter로 전송된 "page"의 값이 Pageable 객체에 적용
        // 적용되는 요청 파라미터 : page, size, sort
    ...
    public String registerBoard(@ModelAttribute BoardDTO boardDTO, List<MultipartFile> files) {
        // @ModelAttribute : form data, request parameter로 전송된 데이터를 객체에 바인딩 해주는 역할
        List<AttachDTO> attachList = new ArrayList<>();

        for(MultipartFile file : files) {
            if(!file.isEmpty()) {
                String originalName = file.getOriginalFilename();
                // UUID : 유니크한 랜덤 아이디를 생성해주는 유틸리티
    ...
    @PostMapping("/edit")
    public String editBoard(Long boardId, BoardDTO boardDTO, List<MultipartFile> files) {
