...
// Business Layer(비지니스 계층) : Controller와 Repository 사이에서 중재 역할
...
public class BoardService {
    ...
    public int saveBoard(BoardDTO boardDTO) {
        ...
        // boardDTO에서 attachDTO 데이터만 attachs로 변경
        ...
        // board 테이블 insert와 attach 테이블의 insert가 한 번에 실행됨(JPA 연결 특성으로 인해)
    ...
    public Page<BoardDTO> getBoards(Pageable pageable) {
        // Pageable : 스프링 Data JPA에서 제공하는 페이징 처리 객체(페이지 번호, 크기, 정렬 방식 등 선택 가능)
        // Page<T> : T 엔티티 형태로 페이징 처리된 데이터를 리턴(실제 데이터 목록, 전체 데이터 수, 전체 페이지 수 등)
        Page<Board> boards = boardRepository.findAll(pageable);
        // Page<Board> findAll(pageable) : 
        // pageable의 페이지 번호, 크기, 정렬 방식 등을 매개변수로 받아와서 페이징 처리 쿼리를 수행하는 메소드
        // 페이징 처리된 데이터는 List<Board> 형태와 동일

        // Page<Board> -> Page<BoardDTO> 수행
        // Page<Board> -> List<Board> 꺼냄 -> List<BoardDTO> 변환 -> Page<BoardDTO>로 구성
        // List.map(method()) : 리스트의 각 데이터에 접근하여 메소드 구조에 맞게 데이터를 매핑하는 역할
    ...
    public BoardDTO getBoardById(Long boardId) {
        ...
        // Optional<Board> : 게시글이 존재하면 Optional 안에 Board 객체가 존재
        //                   존재하지 않으면 Optional.empty()
        // orElseThrow() : Optional 안에 Board 객체가 없으면
    ...
    public int updateBoard(Long boardId, BoardDTO boardDTO) {
        // JPA에서 데이터 수정의 경우 존재하는 데이터를 조회하고 조회된 데이터의 특정 값을 변경한 후에 다시 등록하는 방식(update 없음)
        Board board = boardRepository.findById(boardId)
                                     .orElseThrow(() -> new IllegalArgumentException("게시글이 존재하지 않습니다."));
    ...
    //    CASCADE를 설정하여 부모 테이블 데이터(board)가 삭제되면 자식도 삭제되도록 구성
    @Transactional
