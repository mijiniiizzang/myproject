...
public class Board {
    ...
    // @OneToMany
    // 하나의 게시글(Board)에 여러 개의 첨부파일(Attach)이 연결되는 1:N(일대다) 관계를 의미

    // mappedBy="board"
    // 연관관계의 주인은 Attach 엔티티의 board 필드
    // 실제 외래키는 attach 테이블의 board_id 컬럼이 관리
    ...
    // CascadeType.ALL
    // Board 저장, 수정, 삭제 시 연관된 Attach 엔티티에도 동일한 작업을 전파

    // orphanRemoval=true
    // Board의 attachList에서 제거된 Attach는 DB에서도 자동으로 삭제됨
}
